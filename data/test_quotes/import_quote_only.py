#!/usr/bin/env python3
"""
Import test quote with quote line item only (no repricing, no order creation).
...
5. Uses standard Salesforce Data Tree format

This is a simplified version that only creates the quote without any post-processing.
Use import_quote.py if you also want repricing, order creation, and asset generation.
...
Usage:
    ...
    python import_quote_only.py --org orgfarmorg --quote-name "Custom Quote" --quantity 25
"""
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"✓ Quote import completed successfully!")
    print(f"{'='*60}")
